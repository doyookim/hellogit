...
// req(request) -> 요청객체 : 브라우저가 서버에게 전달되는 정보를 담고 있다.
...
server.on("request", (req, res) => {
  ...
  if (req.url == "/sample.html") {
    ...
    res.writeHead(200, {
      ...
      /** CORS 접근 허용을 위한 설정 */
      // 접급을 허용할 도메인 혹은 IP(브라우저에 출력되고 있는 도메인을 의미함, *은 ALL의미)
    ...
    switch (req.method) {
      case "GET": // 데이터 조회 기능
        json = {
          rt: "OK",
          messange: "GET방식에 대한 요청입니다.",
        };
        break;
      case "POST": // 데이터 저장 기능
        json = {
          rt: "OK",
          messange: "POST방식에 대한 요청입니다.",
        };
        break;
      case "DELETE": // 데이터 삭제
        json = {
          rt: "OK",
          messange: "DELETE방식에 대한 요청입니다.",
        };
        break;
    }
