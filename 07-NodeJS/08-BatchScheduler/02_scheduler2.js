...
schedule.scheduleJob(rule2, () =>
  logger.debug(`[rule2] 매시간 ${rule2.minute}분 ${rule2.second}초 마다 수행!!`)
);
...
schedule.scheduleJob(rule3, () =>
  logger.debug(
    `[rule3] ${rule3.hour}시 ${rule3.minute}분 ${rule3.second}초 마다 수행!!`
  )
);
...
schedule.scheduleJob(rule4, () =>
  logger.debug(`[rule4] 매주 월~토 매 분 ${rule4.second}초마다 실행`)
);
