...
import express, { application, response } from "express"; // Express 본체
...
import methodOverrried from "method-override"; // PUT 파라미터 처리
...
// import multer_error from "multer"; // 업로드 모듈
...
app.use((req, res, next) => {
  ...
  const current_url = urlFormat({
    ...
    host: req.get("hots"), // ex) 172.16.141.1
    pahtname: req.originalUrl, // ex) /page1.html
  });
  ...
  res.on("finish", () => {
    ...
    // 이번 접속에서 클라이언트가 머문 시간 = 백엔트가 실행하는데 걸린 시간
...
| 4) Express 객체의 추가 설정
-----------------------------------------------------*/
/** POST 파라미터 수신 모듈 설정. 추가되는 미들웨어 중 가장 먼저 설정해야 함 */
...
app.use(methodOverrried("X-HTTP-Method")); // Microsoft
app.use(methodOverrried("X-HTTP-Method-Override")); // Google/GData
app.use(methodOverrried("X-Method-Override")); // IBM
// HTTP 폼에서 PUT, DELETE 로 전송할 경우 Post 방식을 사용하되, action 주소에 "?_method" 라고 추가
app.use(methodOverrried("_method")); // HTML form
...
// public/02-get_params_by_js.html
...
router.delete("/send_delelte", (req, res, next) => {
  // URL 파라미터들은 req.body 객체의 하위 데이터로 저장된다.
...
// 위의 형태처럼 개별적인 함수로 구현 가능하지만 대부분 하나의 URL에 메서드 체인을 사용해서 4가지 전송방식을 구현
...
router.route("/upload/single").post((req, res, next) => {
  ...
  upload(req, res, (err) => {
    ...
    // 업로드 결과가 성공이라면 썸네일의 생성 함수를 호출한다.
...
app.listen(process.env.PORT, () => {
  ...
  ip.forEach((v, i) => {
    logger.debug(`server adredd => http://${v}:${process.env.PORT}`);
  });
