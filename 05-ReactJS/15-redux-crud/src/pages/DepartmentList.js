...
const DepartmentList = memo(() => {
  ...
  console.log(data);
  /** 페이지 강제 이동을 처리하기 위한 navaigate함수 생성 */
  ...
  /** 최초 마운트 혹은  QueryString이 변경될 때마다 hook -> 리덕스를 통해 목록을 조회한다. */
  React.useEffect(() => {
    dispatch(
      getList({
        query: query,
    ...
    refRowsDropdown.current.value = query;
            ...
            {data && (
              <Pagenation>
                {data.pagenation.prevGroupLastPage > 0 ? (
                  <li>
                    <NavLink
                      to={`/?query=${query}&rows=${rows}&page=${data.pagenation.prevGroupLastPage}`}
                    >
                      &laquo;
                    </NavLink>
                  </li>
                ...
                {data.pagenation.nextGroupFirstPage > 0 ? (
                  <li>
                    <NavLink
                      to={`/?query=${query}&rows=${rows}&page=${data.pagenation.nextGroupFirstPage}`}
                    >
                      &raquo;
                    </NavLink>
                  </li>
                  ...
                  <li>
                    {/* <NavLink to="#" className="disabled"> */}
                    <NavLink
                      to={`/?query=${query}&rows=${rows}&page=${data.pagenation.groupEnd}`}
                      className="disabled"
                    >
                      &raquo;
                    </NavLink>
