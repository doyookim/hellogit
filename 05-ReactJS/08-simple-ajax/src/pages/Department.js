...
const Department = () => {
  ...
  const onDepartmentSave = React.useCallback((e) => {
    ...
    setTimeout(() => {
      (async () => {
        ...
        if (json != null) {
          // 기존의 상태값과 배열간 병함을 처리하기 위해 응답결과를 배열로 묶음
          const addArr = [json];
          // 배열 병합 결과로 상태값을 갱신함
          // setDepartment(department.concat(addArr));
          // 성능향상을 위해 상태값을 함수형 업데이트로 처림함
  ...
  /** 수정 사항 저장 버튼이 클릭되었을 때 호출될 이벤트 핸들러 */
  // 성능 최적화를 위해 useCallback() 적용함
  const onUpdateSubmit = React.useCallback((e) => {
    ...
    (async () => {
      ...
      if (json !== null) {
        // 함수형 업데이트
        // --> 콜백함수의 파라민터로 상태값의 복사본이 전달되므로 이 값을 직접 수정해도 된다.
        setDepartment((department) => {
          ...
          <tbody>
            {department.length > 0 ? (
              department.map((item, index) => {
                return (
                  <tr key={item.id}>
                    {/* 상태값에 저장되어 있는 수정할 항목의 인덱스에 해당한는 원소라면? */}
                    {updateId === item.id ? (
                      <>
                        {/* 수정을 위한 <input>요소를 표시 */}
                        <td>
