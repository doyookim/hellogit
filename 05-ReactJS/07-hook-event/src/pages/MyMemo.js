...
const MyMemo = () => {
  ...
  /** 특정 값이 반영된 경우만 등장하는 기능을 가정. */
  // myWord를 모니터링하여 이 값이 변경되었을 때 그에 대한 효과로  myLen이라는 상태값을 업데이트 하려는 상황.
  // 1) 출력할 글자의 길이를 상태값으로 정의
  // const [myLen, setMyLen] = React.useState(myWord.length);

  // 2) 미리 비한 상태값이 변경될 수 있는 Effect Hook을 정의
  // React.useEffect(()=>{
