...
function lua_do(si, ai, r, token, doFlag, v) {
    try {
        if (doFlag == true) {
            var rs = "https://nid.naver.com/user2/lua?a=log";
            var index;
            var doc = document;
            ...
            if (doc.images) {
                ...
                doc.write(
                    "<im" + "g sr" + 'c="' + rs + '" width="1" height="1" border="0" />'
                );
            }
        }
    } catch (e) {}
...
function lua_do2(si, ai, r, token, doFlag, v) {
    try {
        if (doFlag == true) {
            var rs = "https://nid.naver.com/user2/lua?m=logB";
            var index;
            var doc = document;
            var wlt = window.location;
...
function lua_getlanguage() {
    ...
    try {
        navigator.userLanguage ?
            (lua_lang = navigator.userLanguage) :
            navigator.language ?
            (lua_lang = navigator.language) :
            "";
    } catch (e) {}
